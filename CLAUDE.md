...
- `docs/` — the Fumadocs docs site (own `npm install`, own deploy). The whole site sits behind a
  ...
  `DOCS_AUTH_SECRET` on the host — unset locally the site runs open, unset in production it fails
  closed with a 503. Never put a real password in `docs/.env.example`; use `docs/.env.local`.
