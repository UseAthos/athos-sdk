---
title: Quickstart
description: From your credentials to a live, scored roleplay call in about 15 minutes.
---

This walks through the entire loop: register a rep, mint a token, start a live call from the
browser, receive the `call.scored` webhook, and read the score. Every REST call is server-to-server —
...
<Callout type="info">
  **Base URL:** `https://app.useathos.ai/api/external/v1`. Paths in these docs are written `/v1/…`
  ...
  API key from your Athos contact — see [Authentication](/authentication).
</Callout>
...
<Callout type="info">
  **Developing on a laptop?** Expose your local handler with a tunnel (ngrok, cloudflared) and give
  ...
  [Registering your endpoint](/backend/webhooks#registering-your-endpoint).

  Your key is a live key: every call runs against the real service and is billed. Keep test data
  apart by registering test reps under a dedicated `platformAgencyId`.
</Callout>
...
own id for them — before their first practice call. An **agency** (one of your customers — a group
of reps) is created for them automatically, named after the rep; rename it any time with
[`PATCH /v1/agencies/:agencyId`](/backend/managing-agencies-and-agents#update-an-agency--patch-v1agenciesagencyid).
...
Registering twice is a `409 AGENT_ALREADY_EXISTS` whose `error.details` carries the existing ids.
Details on [Managing agencies and agents](/backend/managing-agencies-and-agents).
...
<Callout type="warn">
  ...
  browser withholds the microphone. See [Requirements](/sdk#requirements).
</Callout>

`drillKey` picks the scenario: `ma-full-sale` (Medicare Advantage) or `fe-full-sale` (Final
Expense) — see [Drills](/reference/drills). `create()` is synchronous, so register handlers
**before** `connect()` — nothing is missed.
...
<Cards>
  <Card title="Reading calls" href="/backend/reading-calls">
    List a rep's or an agency's history, page through it, and play the recording.
  </Card>
  <Card title="Webhooks" href="/backend/webhooks">
    Retries, deduplication, `success: false`, and reconciling by polling.
  </Card>
  <Card title="SDK events" href="/sdk/events">
    Drive your call UI from the session lifecycle.
  </Card>
