...
Only three codes are raised *before* redemption and leave the token unspent: `INVALID_TOKEN`,
`TOKEN_EXPIRED` and `INVALID_REQUEST`. Of those, only `INVALID_REQUEST` is worth retrying with the
