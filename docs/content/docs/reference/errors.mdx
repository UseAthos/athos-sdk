...
| `AGENT_NOT_FOUND` | 404 | No agent of yours is registered under that id — the id you minted with, or the `agentId` you scoped `GET /calls` by. Register the rep first, or resolve a handle (NPN, email) to an id with `GET /agents`. |
| `AGENCY_NOT_FOUND` | 404 | No such agency for your tenant (also returned for another tenant's agency) — on `PATCH /agencies/{agencyId}`, `GET /agents?agencyId=` and `GET /calls?agencyId=`. Check the `agencyId` you stored. |
...
- **`POST /agents`**, **`GET /agents`** and **`PATCH /agencies/{agencyId}`** (roster):
  `INVALID_REQUEST` (including more than one lookup handle on `GET /agents`), `INVALID_API_KEY`,
  `API_KEY_REVOKED`, `TENANT_INACTIVE`, `IP_NOT_ALLOWED`, `AGENCY_NOT_FOUND` (an `agencyId` that is
  not yours), and `AGENT_ALREADY_EXISTS` (register only). A lookup that matches nobody is a `200`
  with an empty `data`, never an error.
- **`POST /session`** (mint): `INVALID_REQUEST`, `INVALID_API_KEY`, `API_KEY_REVOKED`, `TENANT_INACTIVE`,
  `IP_NOT_ALLOWED`, `TENANT_QUOTA_EXCEEDED`, `AGENT_NOT_FOUND` (the id is not a registered rep's
  `platformAgentId` or Athos `agentId` — an NPN or an email address lands here too).
...
- **`GET /calls` and `GET /calls/:id`** (reads): `INVALID_REQUEST` (list only — a missing or unknown
  `source`, a bad cursor, or the same rep / agency named both ways), `INVALID_API_KEY`,
  `API_KEY_REVOKED`, `TENANT_INACTIVE`, `IP_NOT_ALLOWED`, `AGENCY_NOT_FOUND` / `AGENT_NOT_FOUND`
  (list only — an `agencyId` / `agentId` that is not yours), and `CALL_NOT_FOUND` (detail only). The
  `platformAgencyId` / `platformAgentId` filters never error: an unknown value is an empty page.
