...
- **`POST /agents`**, **`GET /agents`** and **`PATCH /agencies/{agencyId}`** (roster):
  `INVALID_REQUEST`, `INVALID_API_KEY`, `API_KEY_REVOKED`, `TENANT_INACTIVE`, `IP_NOT_ALLOWED`,
  `AGENCY_NOT_FOUND` (an `agencyId` that is not yours), and `AGENT_ALREADY_EXISTS` (register only).
  A lookup that matches nobody is a `200` with an empty `data`, never an error.
