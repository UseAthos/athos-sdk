...
- **`GET /calls` and `GET /calls/:id`** (reads): `INVALID_REQUEST` (list only — a bad cursor, or
  the same rep / agency named both ways), `INVALID_API_KEY`, `API_KEY_REVOKED`, `TENANT_INACTIVE`,
  `IP_NOT_ALLOWED`, `AGENCY_NOT_FOUND` / `AGENT_NOT_FOUND` (list only — an `agencyId` / `agentId`
  that is not yours), and `CALL_NOT_FOUND` (detail only). The `externalAgencyId` /
  `externalUserId` filters never error: an unknown value is an empty page.
