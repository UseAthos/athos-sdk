...
| `AGENCY_NOT_FOUND` | 404 | No such agency for your tenant (also returned for another tenant's agency) — on `PATCH /v1/agencies/:agencyId`, `GET /v1/agents?agencyId=` and `GET /v1/calls?agencyId=`. Check the `agencyId` you stored. |
...
- **`POST /v1/agents`**, **`GET /v1/agents`** and **`PATCH /v1/agencies/:agencyId`** (roster):
  `INVALID_REQUEST` (validation, more than one lookup handle on `GET /v1/agents`, a `limit` outside
  1–100, a bad `cursor`), `INVALID_API_KEY`, `API_KEY_REVOKED`, `TENANT_INACTIVE`, `IP_NOT_ALLOWED`,
  `AGENCY_NOT_FOUND` (an `agencyId` that is not yours), and `AGENT_ALREADY_EXISTS` (register only). A
  lookup that matches nobody is a `200` with an empty `data`, never an error.
