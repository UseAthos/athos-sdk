...
| `AGENT_NOT_FOUND` | 404 | No agent of yours is registered under the id you minted with. Register the rep first, or resolve a handle (NPN, email) to an id with `GET /agents`. |
| `AGENCY_NOT_FOUND` | 404 | No such agency for your tenant (also returned for another tenant's agency). Check the `agencyId` you stored. |
