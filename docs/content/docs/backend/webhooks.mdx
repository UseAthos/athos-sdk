...
[`GET /v1/calls/:id`](/backend/reading-calls). The same body is declared as the `call.scored`
...
- **A real call dropped before it was persisted produces no delivery at all** — too short to
  assess, no usable audio, a transcript that could not be produced, or a rep who is not registered.
  A real call that was persisted but could not be scored arrives as `success: false`. A
  re-disposition of an already-scored real call does **not** fire again — even though it can change
  that call's `score` (including `complianceScorecard`) and `scoredAt`. If you cache scores,
  re-read real calls on a schedule rather than treating the first delivery as final.
...
def verify_athos_signature(raw_body: bytes, header: str, secret: str) -> bool:
    ...
    # Both values are attacker-controlled — check their shape before doing anything with them.
    # (`int()` raises on a non-numeric `t`; `compare_digest` raises on non-ASCII input.)
    if not t or not v1 or not t.isdigit() or not re.fullmatch(r"[0-9a-fA-F]{64}", v1):
        return False
...
- **Polling is the backstop.** If both delivery attempts fail, the event won't be redelivered later —
  ...
  on the call's own time (when a practice call **ended**, when a real call **started**), and scoring
  lands minutes later, so each poll must re-scan a window that **overlaps** the previous one (an
  hour plus the longest real call you expect) and deduplicate on the call `id`. A forward-only
  watermark misses calls that were scored after you read past them.
...
- **`platformAgentId` / `platformAgencyId` can be `null` on a `success: false` event** even for a
  rep you registered under your own id — the ids are resolved at delivery time and a failed attempt
  takes a shorter path. `agentId` / `agencyId` are always present; join on those when yours are
  missing.
