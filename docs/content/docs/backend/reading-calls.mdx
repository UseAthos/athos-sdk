...
```json
{
  ...
  "audioUrlExpiresAt": "2026-06-05T15:40:54.005Z",
  "waveform": null,
...
`audioUrl` is a **short-lived signed URL** — it plays for about an hour and then stops working.
`audioUrlExpiresAt` tells you exactly when. Use it directly in an `<audio>` element or a download
link, but **do not store it** in your database and do not email or embed it anywhere long-lived: when
you need it again, fetch the call again and you get a fresh one. A call with no recording (practised
before recording was enabled) returns `null` for both fields.
