...
[webhook delivery](/backend/webhooks#responding-retries-and-idempotency)), re-scan a window that
...
| `compliancePercent` | number *(optional)* | How compliant the call was, as a percentage **0–100** (higher is better). Present whenever the call was assessed for compliance, whichever scorecard graded it. |
| `complianceScorecard` | object *(optional)* | The **default** Medicare rubric's compliance review, section by section, headed by a `totalScore` on **that rubric's own point scale** — not a percentage. |
...
<Callout type="info">
  **Two different fields.** `compliancePercent` answers *how compliant was this call* — a percentage
  **0–100**, comparable across calls whichever scorecard graded them, and the one to render.
  `complianceScorecard` is the **default** Medicare rubric's section-by-section detail, and its
  `totalScore` is a raw total on that rubric's own point scale — **not** a percentage. A call can
  carry `compliancePercent` on its own.
  ...
  **`complianceScorecard` is optional — check for it, don't assume it.** On practice calls it appears
  ...
  graded content and may gain items over time — read `compliancePercent` and treat the rest as
  display detail. `totalScore` is itself omitted in the rare case where the underlying total is not a
  usable number; treat an absent `totalScore` as *not assessed*, never as zero.
</Callout>
