...
one only if that retry says the token is gone. A session whose `connect()` *failed* goes back to
idle and can be reused — but a session you cancelled with `disconnect()`, or one whose call actually
ran, is finished. Creating a fresh session per attempt is the rule that always holds:
