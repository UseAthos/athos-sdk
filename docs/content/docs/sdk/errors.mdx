...
<Callout type="info">
  **`connect()` asks for the microphone before it redeems the token**, so a denied or missing
  ...
  same token on a *new* session; if that returns `TOKEN_ALREADY_USED`, the failure came from later
  in the call setup — mint a fresh token. See
...
`AGENT_NOT_FOUND`, `TENANT_INACTIVE` and `TENANT_QUOTA_EXCEEDED` never reach the browser. All three
come back on your backend's server-to-server calls — `AGENT_NOT_FOUND` (`404`) and
`TENANT_QUOTA_EXCEEDED` (`403`) when you [mint a token](/backend/minting-tokens) for a rep who is not
registered or once the allowance is spent, `TENANT_INACTIVE` (`403`) on any API-key call — so handle
them there, not on the SDK's `error` channel.
