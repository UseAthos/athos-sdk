...
`TENANT_INACTIVE` and `TENANT_QUOTA_EXCEEDED` never reach the browser. They are `403`s from your
backend's own call to `POST /v1/session`, so you'll see them when you [mint the
token](/backend/minting-tokens), not on the SDK's `error` channel.
