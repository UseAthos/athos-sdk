...
<Callout type="info">
  **The microphone codes are raised before the token is redeemed.** `connect()` asks for the mic
  first, so a denied or missing microphone leaves the token **unspent** — recover by fixing the
  device and retrying with the same token. See
...
| `SERVICE_UNAVAILABLE` | Athos is temporarily unavailable. | Mint a new token and start a new call — the token is spent. |
| `INTERNAL_ERROR` | An unexpected error. | Mint a new token and start a new call; if it persists, contact Athos with the message. |

<Callout type="warn">
  **A failed redemption usually spends the token.** It is consumed as the request is redeemed, so
  only `INVALID_TOKEN`, `TOKEN_EXPIRED` and `INVALID_REQUEST` leave it unspent — every other failure
  needs a newly minted token, not a replay. Full rule:
  [error reference](/reference/errors#a-failed-redemption-usually-spends-the-token).
</Callout>

`TENANT_INACTIVE` and `TENANT_QUOTA_EXCEEDED` never reach the browser. Both are `403`s on your
backend's server-to-server calls — `TENANT_QUOTA_EXCEEDED` when you [mint a
token](/backend/minting-tokens), `TENANT_INACTIVE` on any API-key call — so handle them there, not
on the SDK's `error` channel.
